<template>
  <div>
    <table-custom :columns="columns" :tableData="tableData" :hasToolbar="false" :hasPagination="false">
      <template #name="{ rows }">
      ...
      <template #operator="{ rows, index }">
        ...
        <template v-else>
          <el-button type="success" size="small" icon="Select" @click="rows.editing = false"> 保存 </el-button>
          <el-button type="default" size="small" icon="CloseBold" @click="handleCancel(rows, index)"> 取消 </el-button>
        </template>
      </template>
    </table-custom>
  </div>
